...
function gunLabel(item) {
  return [item.make, item.model].filter(Boolean).join(' ') || 'Unknown';
}
...
function StatCard({ icon: Icon, label, value, color = '#c8a96e', sub }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 32, fontWeight: 800, lineHeight: 1, letterSpacing: '-0.03em', color: 'var(--text-primary)' }}>
        {value}
      </div>
...
// Clean horizontal bar chart — no recharts, pure CSS
        ...
        return (
          <div key={label}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, fontFamily: 'var(--font-mono)', color: col, flexShrink: 0, marginLeft: 8 }}>{count}</span>
            </div>
...
// Clickable firearm row for timeline sections
    ...
    >
      ...
      <div style={{ flexShrink: 0, textAlign: 'right' }}>
        ...
        {item.status && item.status !== 'Active' && (
          <span className={`badge badge-${item.status.toLowerCase()}`} style={{ fontSize: 10, marginTop: 2 }}>
            {item.status}
          </span>
        )}
      </div>
  ...
  return (
    <div className="page-container">
      ...
      <div className="stat-grid" style={{
        ...
        <StatCard icon={Shield}  label="Active"   value={summary.active_items || 0} color="#5a9e6e"
          sub={inactive > 0 ? `${inactive} inactive` : undefined} />
        <StatCard icon={Users}   label="Owners"   value={summary.total_owners || 0} color="#4a7fa8" />
        {by_category.map((c, i) => (
          <StatCard key={c.category} icon={Activity}
          ...
          {least_makes.filter(m => !top_makes.slice(0,3).find(t => t.make === m.make)).length > 0 && (
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 10 }}>
                <TrendingDown size={11} color="var(--text-muted)" />
                <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>Least</span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {least_makes.filter(m => !top_makes.slice(0,3).find(t => t.make === m.make)).map(m => (
                  <div key={m.make} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 10px', background: 'var(--bg-elevated)', borderRadius: 6 }}>
                    ...
                    <span style={{ fontSize: 12, fontFamily: 'var(--font-mono)', color: 'var(--text-muted)' }}>{m.count}×</span>
                  </div>
                ))}
              </div>
            </>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {least_calibers.filter(c => !top_calibers.slice(0,3).find(t => t.caliber === c.caliber)).map(c => (
                  <div key={c.caliber} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 10px', background: 'var(--bg-elevated)', borderRadius: 6 }}>
                    ...
                    <span style={{ fontSize: 12, fontFamily: 'var(--font-mono)', color: 'var(--text-muted)' }}>{c.count}×</span>
                  </div>
                ))}
              </div>
            </>
        ...
        <Panel>
          ...
          {!recently_acquired.length
            ...
            : recently_acquired.map(item => (
                <FirearmRow key={item.id} item={item} right={fmtDate(item.purchase_date)} onClick={() => navigate(`/inventory/${item.id}`)} />
              ))
          }
        </Panel>

        <Panel>
          <SectionHead icon={Clock} title="Longest Owned" sub="Oldest by purchase date, active only" />
          {!longest_owned.length
            ? <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>No purchase dates recorded.</p>
            : longest_owned.map(item => (
                <FirearmRow key={item.id} item={item} right={ageLabel(item.years_owned, item.months_owned)} onClick={() => navigate(`/inventory/${item.id}`)} />
              ))
          }
        </Panel>
                      ...
                      return (
                        <div key={row.status}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                            ...
                            <span style={{ fontSize: 12, fontWeight: 700, fontFamily: 'var(--font-mono)', color: col }}>{row.count}</span>
                          </div>
                          <div style={{ height: 6, background: 'var(--bg-elevated)', borderRadius: 3, overflow: 'hidden' }}>
      ...
      <Panel style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)' }}>
          <SectionHead icon={Activity} title="Recently Added to System" style={{ marginBottom: 0 }} />
        </div>
        <div className="table-wrap">
          <table>
            ...
            <tbody>
              {!recent_items.length
                ? <tr><td colSpan={6} style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '28px 0' }}>No items yet.</td></tr>
                : recent_items.map(item => (
                    <tr key={item.id} onClick={() => navigate(`/inventory/${item.id}`)} style={{ cursor: 'pointer' }}>
                      <td style={{ fontWeight: 600 }}>{gunLabel(item)}</td>
                      <td style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-secondary)' }}>{item.caliber || '—'}</td>
                      ...
                      <td style={{ fontSize: 11, color: 'var(--text-muted)' }}>{fmtDate(item.created_at)}</td>
                    </tr>
                  ))
              }
            </tbody>
