...
import { itemsApi, locationsApi, categoriesApi, tagsApi, exportApi } from '../api/client';
...
export default function InventoryPage() {
  ...
  const [filters, setFilters] = useState({
    search: '', status: '', category_id: '', location_id: '', tag: '', sort_by: 'created_at', sort_dir: 'desc',
  });
  ...
  const clearFilters = () => setFilters({ search: '', status: '', category_id: '', location_id: '', tag: '', sort_by: 'created_at', sort_dir: 'desc' });

  const handleSort = (col) => setFilters(f => ({
    ...f, sort_by: col, sort_dir: f.sort_by === col && f.sort_dir === 'desc' ? 'asc' : 'desc',
  }));

  const SortIcon = ({ col }) => {
    if (filters.sort_by !== col) return null;
    return filters.sort_dir === 'asc' ? <ChevronUp size={12} /> : <ChevronDown size={12} />;
  };
    ...
    <div className="page-container">
      ...
      <div className="card" style={{ marginBottom: 16, padding: '14px 16px' }}>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <select value={filters.category_id} onChange={e => setFilter('category_id', e.target.value)} style={{ flex: '0 1 130px' }}>
            <option value="">All Types</option>
            {categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>

          <select value={filters.location_id} onChange={e => setFilter('location_id', e.target.value)} style={{ flex: '0 1 140px' }}>
            <option value="">All Locations</option>
            {locations.map(l => <option key={l.id} value={l.id}>{l.name}</option>)}
          </select>

          {/* Tag filter */}
          <select value={filters.tag} onChange={e => setFilter('tag', e.target.value)} style={{ flex: '0 1 130px' }}>
            <option value="">All Tags</option>
            {tags.map(t => <option key={t.id} value={t.name}>{t.name}</option>)}
          </select>

          {hasFilters && (
            <Btn variant="ghost" size="sm" onClick={clearFilters}><X size={13} /> Clear</Btn>
          )}
        </div>
              ...
              <thead>
                <tr>
                  ...
                  <th onClick={() => handleSort('value')} style={{ cursor: 'pointer', userSelect: 'none' }}>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>Value <SortIcon col="value" /></span>
                  </th>
                  <th>Status</th>
                ...
                {items.map(item => (
                  <tr key={item.id} onClick={() => navigate(`/inventory/${item.id}`)} style={{ cursor: 'pointer' }}>
                    <td>
                      ...
                      {item.serial_number && (
                        <div style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', marginTop: 2 }}>S/N: {item.serial_number}</div>
                      )}
                      {item.is_private && (
